...
* Call Check-NamedProps against the mailbox you want to fix (note that this can take a long time to run).  The script supports wild card name searches, so if you want to find all named properties that start with 'badproperty', you can search for 'badproperty*': `Check-NamedProps -Mailbox "badnamedprops@e19.local" -restart -dumpentryids -powershellurl http://e1.e19.local/PowerShell/ -SearchNamedProp "badnamedprop*"`
